...
import { PlaceType, CreateJourneyPlace, CreateJourneyDay } from '@/types/journey.types';
...
export const useJourneyForm = (): UseJourneyFormReturn => {
  ...
  const [journeyPlaces, setJourneyPlaces] = useState<{ [key: string]: CreateJourneyPlace[] }>({
    "Day 1": []
  });
  const [expandedPlaces, setExpandedPlaces] = useState<{ [key: string]: boolean }>({});
  ...
  const deleteDay = useCallback((dayToDelete: string) => {
  ...
  const getDateForDay = useCallback((dayLabel: string): string => {
  ...
  const getPlacesByType = useCallback((type: PlaceType) => {
  ...
  const addPlaceToActiveDay = useCallback((type: PlaceType) => {
  ...
  const removePlaceFromActiveDay = useCallback((index: number) => {
    setJourneyPlaces(prev => ({
      ...prev,
  ...
  const updatePlaceField = useCallback((index: number, field: keyof CreateJourneyPlace, value: string | number) => {
    setJourneyPlaces(prev => ({
      ...prev,
  ...
  const togglePlaceExpansion = useCallback((dayKey: string, placeIndex: number) => {
  ...
  const isPlaceExpanded = useCallback((dayKey: string, placeIndex: number) => {
    const key = `${dayKey}-${placeIndex}`;
  ...
  const submitJourney = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      const journeyDays: CreateJourneyDay[] = days.map((dayLabel, index) => {
        ...
        dayDate.setDate(formData.startDate.getDate() + index);
