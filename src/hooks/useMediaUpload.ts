...
import { uploadToS3, uploadMultipleToS3, deleteFromS3, UploadOptions, UploadResult, UploadProgress } from '@/lib/aws/media-upload';
...
export const useMediaUpload = (): UseMediaUploadReturn => {
  ...
  // Upload single file
  const uploadSingle = useCallback(async (file: File, options: UploadOptions = {}): Promise<UploadResult> => {
    ...
    try {
      const results = await uploadMultipleToS3(
        ...
        (fileIndex: number, progress: UploadProgress) => {
          const fileKey = `${files[fileIndex].name}-${Date.now()}-${fileIndex}`;
          setState(prev => ({
            ...prev,
            uploadProgress: {
              ...prev.uploadProgress,
      ...
      setState(prev => ({
        ...prev,
        isUploading: false,
        errors: { ...prev.errors, general: error.message || 'Multiple upload failed' },
      }));
    ...
    try {
      ...
      if (result.success) {
        ...
        setState(prev => {
          ...
          const resultKey = Object.keys(newResults).find(k => newResults[k].key === key);
          if (resultKey) {
  ...
  const getUploadedUrl = useCallback((fileKey: string): string | undefined => {
    return state.results[fileKey]?.url;
  }, [state.results]);

  // Check if file is uploaded
  const isFileUploaded = useCallback((fileKey: string): boolean => {
    return state.results[fileKey]?.success === true;
  }, [state.results]);

  // Get file progress
  const getFileProgress = useCallback((fileKey: string): UploadProgress | undefined => {
