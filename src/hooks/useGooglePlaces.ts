...
export interface UseGooglePlacesReturn {
  ...
  handlePlaceSelect: (placeId: string, description: string, onLocationFound: (lat: number, lng: number, address: string, name: string) => void) => void;
  hideSuggestions: (fieldKey: string) => void;
}

export const useGooglePlaces = (): UseGooglePlacesReturn => {
  const [placeSuggestions, setPlaceSuggestions] = useState<{ [key: string]: PlaceSuggestion[] }>({});
  const [showSuggestions, setShowSuggestions] = useState<{ [key: string]: boolean }>({});
  const [searchTimeouts, setSearchTimeouts] = useState<{ [key: string]: NodeJS.Timeout }>({});

  const handleAutocompletePredictions = useCallback((query: string, fieldKey: string) => {
  ...
  const handlePlaceSelect = useCallback((
    placeId: string, 
    description: string, 
    onLocationFound: (lat: number, lng: number, address: string, name: string) => void
  ) => {
