...
export interface UseNearbyJourneysReturn {
  ...
  fetchByLocation: (coordinates: LocationCoordinates, radius?: number, limit?: number) => Promise<void>;
  clearError: () => void;
...
export function useNearbyJourneys(
  ...
  const [lastParams, setLastParams] = useState<NearbyJourneysParams | null>(null);

  const clearError = useCallback(() => {
    setError(null);
  }, []);

  const fetchNearbyJourneys = useCallback(async (params: NearbyJourneysParams) => {
    try {
      setIsLoading(true);
      setError(null);
      ...
      if (response.statusCode === 200 && response.data) {
        ...
        throw new Error(response.message || 'Failed to fetch nearby journeys');
      }
    } catch (err: any) {
