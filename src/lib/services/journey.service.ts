...
export class JourneyService implements IJourneyService {
  ...
  async getMyJourneys(filters?: JourneyFilters): Promise<Journey[]> {
    try {
      ...
      const errorMessage = error?.response?.data?.message || error?.message || 'Failed to fetch my journeys';
  ...
  async getAllJourneys(filters?: JourneyFilters): Promise<Journey[]> {
    try {
      ...
      const errorMessage = error?.response?.data?.message || error?.message || 'Failed to fetch public journeys';
      throw new Error(errorMessage);
    }
  }
  ...
  async getJourneyById(id: string): Promise<Journey> {
    try {
      ...
      const errorMessage = error?.response?.data?.message || error?.message || 'Failed to fetch journey details';
      throw new Error(errorMessage);
    }
  }
  ...
  async updateJourney(id: string, data: UpdateJourneyDto): Promise<Journey> {
    try {
      ...
      const errorMessage = error?.response?.data?.message || error?.message || 'Failed to update journey';
      throw new Error(errorMessage);
    }
  }
  ...
  async deleteJourney(id: string): Promise<void> {
    try {
      ...
      const errorMessage = error?.response?.data?.message || error?.message || 'Failed to delete journey';
      throw new Error(errorMessage);
    }
  }
    ...
    try {
      ...
      const stats: JourneyStats = {
        ...
        totalPlaces: journeys.reduce((total, journey) => {
          return total + (journey.days?.reduce((dayTotal, day) => {
      ...
      const errorMessage = error?.message || 'Failed to fetch journey statistics';
      throw new Error(errorMessage);
    }
  }
  ...
  async removeDayFromJourney(id: string, dayId: string): Promise<DetailedJourney> {
    try {
      ...
      const errorMessage = error?.message || 'Failed to remove day from journey';
      throw new Error(errorMessage);
    }
  }
  ...
  private createDefaultBanner(journey: Journey): JourneyBanner {
    return {
      ...
      imageUrl: 'https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=1200&h=600&fit=crop',
      overlayOpacity: 0.4,
      textPosition: 'center',
      textColor: '#ffffff',
    };
