...
export const MediaManager: React.FC<MediaManagerProps> = ({
  ...
  const updateMediaItems = useCallback((items: MediaItem[]) => {
    setMediaItems(items);
    onMediaChange?.(items);
  }, [onMediaChange]);
  ...
  const handleReorder = useCallback((reorderedItems: MediaItem[]) => {
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-medium text-gray-900">
                {fileType === 'images' ? 'Images' : fileType === 'videos' ? 'Videos' : 'Documents'}
              </h3>
          ...
          {!canUploadMore && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
              <div className="flex">
                ...
                <div>
                  <p className="text-sm text-amber-800 font-medium">Upload limit reached</p>
                  <p className="text-xs text-amber-700">
                    You have reached the maximum of {maxFiles} files. Delete some files to upload more.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div>
          {mediaItems.length > 0 ? (
            ...
            <div className="text-center py-12 text-gray-500">
              <svg className="mx-auto w-12 h-12 text-gray-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 48 48">
